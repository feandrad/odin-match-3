...
init_board :: proc() -> Board {
    ...
    for y in 0..<GRID_HEIGHT {
        for x in 0..<GRID_WIDTH {
            r := rl.GetRandomValue(i32(GemType.Red), i32(GemType.White))
            b.slots[y][x] = GemType(r)
        }
    }
    return b
}
...
set_gem :: proc(b: ^Board, p: GridPosition, g: GemType) {
    if is_in_bounds(p) { b.slots[p.y][p.x] = g }
}
...
swap_gems :: proc(b: ^Board, a, c: GridPosition) {
    if !is_in_bounds(a) || !is_in_bounds(c) { return }
    tmp := get_gem(b^, a)
    set_gem(b, a, get_gem(b^, c))
    set_gem(b, c, tmp)
}
...
draw_board :: proc(b: Board, s: DragState) {
    ...
    for y in 0..<GRID_HEIGHT {
        for x in 0..<GRID_WIDTH {
            if s.dragging && x == s.start_grid.x && y == s.start_grid.y {
                continue
            }
            g := b.slots[y][x]
            if g == .None { continue }
